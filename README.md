...
# 3. الإعداد التلقائي (يولد AUTH_SECRET + ينشئ الجداول + يزرع البيانات)
npm run setup
...
3. من جهازك المحلي فقط (منفذ 5432 المباشر — بدون pgbouncer):
   ```bash
...
│   ل── providers.tsx        # SessionProvider
├── lib/
│   ├── auth.ts              # NextAuth + requireSession / requireRole
│   ل── prisma.ts            # Prisma client singleton
├── services/
...
│   ل── whatsapp-service.ts    # رسائل wa.me + تطبيع أرقام الجوال
ل── types/
    ├── index.ts             # أنواع مشتركة (ProjectStatus, UserRole, ...)
    ├── deduction.ts         # IDeductionProfile + Zod schema
    ل── next-auth.d.ts       # تمديد أنواع JWT/Session
prisma/
├── schema.prisma            # نموذج قاعدة البيانات (PostgreSQL)
ل── seed.ts                  # بيانات أولية: مصنع تجريبي + 5 بروفايلات + مخزون
```
