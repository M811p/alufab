 ...
 * نقل المشروع لحالة جديدة وفق آلة الحالات وصلاحيات الدور.
 * الخصم/الإرجاع المخزني يتم تلقائياً داخل ProjectWorkflow.
 */
export async function PATCH(req: NextRequest, { params }: RouteParams) {
  try {
    ...
    const result = await ProjectWorkflow.transition({
      projectId: id,
      tenantId: actor.tenantId, // العزل بين المستأجرين من الجلسة — لا يُقبل من العميل أبداً
 ...
 * يُرجع الحالة الحالية والانتقالات المتاحة لدور المستخدم الحالي —
 * تستخدمها الواجهة لإظهار/إخفاء أزرار سير العمل.
 */
