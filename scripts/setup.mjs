...
const fail = (msg) => { console.error(`\x1b[31m[setup] ✗ ${msg}\x1b[0m`); process.exit(1); };
const run = (cmd) => { log(`$ ${cmd}`); execSync(cmd, { stdio: 'inherit' }); };
...
if (env.includes('USER:PASSWORD@HOST')) {
  fail('عدّل DATABASE_URL في ملف .env أولاً ثم أعد تشغيل: npm run setup');
}
