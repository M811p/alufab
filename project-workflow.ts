...
/**
 * آلة الحالات (State Machine) لدورة حياة المشروع:
 * DRAFT → QUOTED → APPROVED → PRODUCTION → INSTALLATION → COMPLETED
 *
 * انتقالات عكسية مسموحة (للمراجعة فقط):
 * QUOTED → DRAFT      (إعادة فتح التسعير)
 * APPROVED → QUOTED   (سحب الاعتماد — يُعاد المخزون تلقائياً)
 */
...
/** الأدوار المخوّلة لكل انتقال — مفتاح الانتقال بصيغة FROM>TO */
...
export interface TransitionContext {
  ...
  actorRole: UserRole;
  /** عند true: يُسمح بالاعتماد حتى لو كان المخزون غير كافٍ (يُسجَّل عجز) */
...
export class ProjectWorkflow {
  ...
  public static async transition(ctx: TransitionContext): Promise<TransitionResult> {
    const project = await prisma.project.findFirst({
      where: { id: ctx.projectId, tenantId: ctx.tenantId }, // عزل المستأجرين إلزامي
      include: {
        quotations: {
    ...
    // ===== شروط مسبقة وخطافات (Hooks) لكل انتقال =====
    ...
    if (from === 'APPROVED' && to === 'QUOTED') {
      // سحب الاعتماد: إعادة الكميات المخصومة للمخزون
      if (latestQuotation?.bom) {
  ...
  /** يفحص بنود الـ BOM ضد أرصدة المخزون ويُرجع قائمة العجز */
  private static async checkStockSufficiency(
    ...
    for (const item of bomItems) {
      ...
      // الأصناف غير المسجلة بالمستودع لا تُحتسب عجزاً (مواد تُشترى حسب الطلب)
